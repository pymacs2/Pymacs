...
Pymacs' tests, and ultimately checks that you can invoke Pymacs with ~python -c "import Pymacs"~.
...
The changes are:
 ...
 - There is a new (much simpler for most people) way to install Pymacs, ~install-pymacs.sh~.  It performs the combined setup tasks that were the subject of
