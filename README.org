...
Pymacs' tests, and ultimately checks that you can invoke Pymacs with
src_python[:exports code]{python -c "import Pymacs"}.
...
The changes are:
 ...
 - There is a new (much simpler for most people) way to install Pymacs, src_python[:exports
   code]{install-pymacs.sh}.  It performs the combined setup tasks that were the subject of
   many blog posts.  If I missed something, please let me know or create a pull request.
